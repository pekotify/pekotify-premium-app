...
export default function AdminTable() {
    ...
    const getSubscriptions = async () => {
        try {
            const resp = await fetch('http://localhost:3010/requests', {
                method: 'GET',
                headers: {
    ...
    const sendApproval = async (creator_id, subscriber_id, status) => {
        console.log('http://localhost:3010/approve/' + status + "/" + String(creator_id) + "/" + String(subscriber_id))
        const resp = await fetch('http://localhost:3010/approve/' + status + "/" + String(creator_id) + "/" + String(subscriber_id), {
            method: 'GET',
            headers: {
                'Authorization': 'Bearer ' + token,
                'Content-Type': 'application/json',
